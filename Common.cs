...
public static class Functions
{
    ...
    public static ItemInfo1 GetRealItem1(ItemInfo1 Origin, ushort Level, MirClass job, List<ItemInfo1> ItemList)
    {
        if (Origin.ClassBased && Origin.LevelBased)
            return GetClassAndLevelBasedItem1(Origin, job, Level, ItemList);
        if (Origin.ClassBased)
            return GetClassBasedItem1(Origin, job, ItemList);
        if (Origin.LevelBased)
            return GetLevelBasedItem1(Origin, Level, ItemList);
        return Origin;
    }
    ...
    public static ItemInfo1 GetLevelBasedItem1(ItemInfo1 Origin, ushort level, List<ItemInfo1> ItemList)
    {
        ItemInfo1 output = Origin;
        for (int i = 0; i < ItemList.Count; i++)
        {
            ItemInfo1 info = ItemList[i];
            if (info.Name.StartsWith(Origin.Name))
                if ((info.RequiredType == RequiredType1.Level) && (info.RequiredAmount <= level) && (output.RequiredAmount < info.RequiredAmount) && (Origin.RequiredGender == info.RequiredGender))
                    output = info;
        }
        return output;
    }
    ...
    public static ItemInfo1 GetClassBasedItem1(ItemInfo1 Origin, MirClass job, List<ItemInfo1> ItemList)
    {
        for (int i = 0; i < ItemList.Count; i++)
        {
            ItemInfo1 info = ItemList[i];
    ...
    public static ItemInfo1 GetClassAndLevelBasedItem1(ItemInfo1 Origin, MirClass job, ushort level, List<ItemInfo1> ItemList)
    {
        ItemInfo1 output = Origin;
        for (int i = 0; i < ItemList.Count; i++)
        {
...
public class ItemInfo
{
    ...
    public static ItemInfo FromText(string text)
    {
        ...
        if (!bool.TryParse(data[88], out info.AllowLvlSys))
            return null;
        if (!bool.TryParse(data[89], out info.AllowRandomStats))
            return null;
        return info;
...
public class ItemInfo1
{
    ...
    public ItemType Type;
    public ItemGrade Grade;
    public RequiredType1 RequiredType = RequiredType1.Level;
    public RequiredClass RequiredClass = RequiredClass.None;
    public RequiredGender RequiredGender = RequiredGender.None;
    public ItemSet Set;
    ...
    public static ItemInfo1 CloneItem(ItemInfo1 item)
    {
        ItemInfo1 Clone = new ItemInfo1
        {
            Index = item.Index,
            Accuracy = item.Accuracy,
            Agility = item.Agility,
    ...
    public void UpdateItem(ItemInfo1 item)
    {
        UpdateToName = item.UpdateToName;
        Accuracy = item.Accuracy;
        Agility = item.Agility;
    ...
    public ItemInfo1(BinaryReader reader, int version = int.MaxValue, int Customversion = int.MaxValue, bool client = false)
    {
        Index = reader.ReadInt32();
        Name = reader.ReadString();
        Type = (ItemType)reader.ReadByte();
        if (version >= 40) Grade = (ItemGrade)reader.ReadByte();
        RequiredType = (RequiredType1)reader.ReadByte();
        RequiredClass = (RequiredClass)reader.ReadByte();
        RequiredGender = (RequiredGender)reader.ReadByte();
        if (version >= 17) Set = (ItemSet)reader.ReadByte();
    ...
    public static ItemInfo1 FromText(string text)
    {
        string[] data = text.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);

        if (data.Length < 69) return null;

        ItemInfo1 info = new ItemInfo1 { Name = data[0] };
...
public class UserItem1
{
    public ulong UniqueID;
    public int ItemIndex;

    public ItemInfo1 Info;
    public ushort CurrentDura, MaxDura;
    public uint Count = 1, GemCount = 0;
    public ushort MinAC, MaxAC;
    public ushort MinMAC, MaxMAC;
    ...
    //  Gem | Orb cap = 5
    public UserItem1[] Slots = new UserItem1[5];
    ...
    public UserItem1(ItemInfo1 info)
    {
        SoulBoundId = -1;
        ItemIndex = info.Index;
        Info = info;
    ...
    public UserItem1(BinaryReader reader, int version = int.MaxValue, int Customversion = int.MaxValue)
    {
        UniqueID = reader.ReadUInt64();
        ItemIndex = reader.ReadInt32();
        ...
        {
            if (reader.ReadBoolean()) continue;
            UserItem1 item = new UserItem1(reader, version, Customversion);
            Slots[i] = item;
        }
    ...
    public void ItemLevelUp(ushort level, MirClass job, MirGender gender, List<ItemInfo1> list)
    {
        ItemInfo1 realItem = Functions.GetRealItem1(Info, level, job, list);
        if (Info.AllowRandomStats)
        {
    ...
    public UserItem1 Clone()
    {
        UserItem1 item = new UserItem1(Info)
        {
            UniqueID = UniqueID,
            CurrentDura = CurrentDura,
            MaxDura = MaxDura,
...
public class Awake
{
    ...
    public byte DC => (type == AwakeType.DC ? AwakeValue : (byte)0);
    public byte getDC() { return (type == AwakeType.DC ? getAwakeValue() : (byte)0); }
    public byte getMC() { return (type == AwakeType.MC ? AwakeValue : (byte)0); }
...
public class Awake1
{
    ...
    public Awake1(BinaryReader reader)
    {
        type = (AwakeType)reader.ReadByte();
        int count = reader.ReadInt32();
        for (int i = 0; i < count; i++)
...
public class ClientMagic
{
    ...
    public string[] SpellString = new string[]
{
        "None",// = 0,
