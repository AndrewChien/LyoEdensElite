...
namespace Server.MirEnvir
{
    ...
    public class Map
    {
        ...
        {
            //首先根据地图尺寸创建了 Cells，然后根据每个瓦片的属性为 Cell 填充属性
            int offSet = 0;
        ...
        public bool Load()
        {
            //瓦片地图的加载过程，其中包含动态部分和静态部分，静态部分包含了地图尺寸和瓦片类型，
            //动态部分上面只涵盖了安全区、刷怪点、传送点和道具限制等这些基础属性，
            //除此之外，地图上的玩家, 怪物和 NPC 等对象才是地图的核心数据。

            //运行时的 Tilemap Cell 创建是在上述代码中的 LoadMapCellsX 中完成的，
            //这里我们以 LoadMapCellsv0 为例，可以看到这里首先根据地图尺寸创建了 Cells，
            //然后根据每个瓦片的属性为 Cell 填充属性
            try
            {
                ...
                if (File.Exists(fileName))
                {
                    ...
                    //以 NPC 为例，NPC 的加载是在地图加载的第二阶段，
                    //在完成了 LoadMapCellsv0 后会根据一阶段 MapInfo 中的 NPC 配置将 NPC 添加到特定的 Cell 中
                    for (int i = 0; i < Info.NPCs.Count; i++)
                    {
                        NPCInfo info = Info.NPCs[i];
                        //这里的 ValidPoint 指的是该坐标点是否可通行，
                        //在后续处理人物行走等逻辑时也会用到，它的判断依据是瓦片属性是否等于 Walk
                        if (!ValidPoint(info.Location)) 
                            continue;

                        //上述代码在检查目标点合法后，会通过 AddObject 方法将 NPC 对象添加到地图中，
                        //传奇的瓦片地图上的每一个对象都需要继承 MapObject 来维护对象在地图上的状态
    ...
    /// <summary>
    /// Map 在处理 AddObject 时会单独维护 Player 与 NPC 的列表，
    /// 随后获取到当前坐标点的 Cell 完成对象的添加逻辑，
    /// Cell 本身的逻辑很轻，只维护对象列表而不处理任何逻辑
    /// </summary>
    public class Cell
    {
    ...
    /// <summary>
    /// 在服务端启动过程中，MapRespawn.RespawnTime 和 MapRespawn.NextSpawnTick 都是 0
    /// （对于高版本的 DB，服务端也可以选择在 Shutdown 时将这两个数据序列化，并在下次启动时恢复），
    /// 因此每个刷怪点都会触发一次刷新逻辑，即调用到 MapRespawn.Spawn。
    /// </summary>
