...
namespace Server.MirEnvir
{
    ...
    public class Envir
    {
        ...
        public const string DatabasePath = @".\Server.MirDB";//注意伊甸园版本没有使用这个整体数据库，而是使用了分体数据库
            ...
            lock (LoadLock)
            {
                ...
                using(FileStream stream = File.OpenRead(Settings.DBPath + "Map.db"))
                {
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        ...
                        {
                            //从Map.db加载地图列表，此处加载的是地图索引以及动态配置，例如刷怪点和传送点，其中的 FileName 指向的是第二阶段加载的地图元数据
        ...
        private void StartEnvir()
        {
            ...
            //LoadDB 中，加载的是地图索引以及动态配置，例如刷怪点和传送点，其中的 FileName 指向的是第二阶段加载的地图元数据
            LoadDB();
            LoadItemInfoFile();

            for (int i = 0; i < MapInfoList.Count; i++)
            {
                //在第二阶段 CreateMap 中，加载的是瓦片地图的元数据，
                //这部分数据应该是美术和策划在地图编辑器中完成地图制作后导出的静态资源文件，后期无需再进行修改
