    ...
    public class MirConnection
    {
        ...
        private void ProcessPacket(Packet p)
        {
            ...
            for (int i = 0; i < data.Count; i++)
                dataLen += data[i];*/
            //SMain.EnqueuePacketLog(string.Format("[Packet Recived] {0} with length {1}", (ClientPacketIds)p.Index, dataLen));
            switch (p.Index)
            {
                ...
                case (short)ClientPacketIds.DeleteCharacter:
                    ...
                    break;
                case (short)ClientPacketIds.StartGame:  //进入游戏请求包
                ...
                case (short)ClientPacketIds.InventoryToHeroInventory:
                    InventoryToHeroInventory((C.InventoryToHeroInventory)p);
                    break;
                ...
                case (short)ClientPacketIds.HeroInventoryToInventory:
                    HeroInventoryToInventory((C.HeroInventoryToInventory)p);
                    break;
                ...
                case (short)ClientPacketIds.HeroMagicKey:
                    HeroMagicKey((C.HeroMagicKey)p);
                    break;
                case (short)ClientPacketIds.AutoPotUpdate:
                    AutoPotUpdate((C.AutoPotUpdate)p);
                    break;
                ...
                case (short)ClientPacketIds.MarketRefresh:
                    MarketRefresh((C.MarketRefresh)p);
                    return;
        ...
        public void GetLMSMatches(C.Get_BR_Matches p)
        {
            if (Stage != GameStage.Game)
                return;
            Player.Get_LMS_BR_Maps();
        }

        public void LMS_Signup(C.BR_Signup p)
        {
            if (Stage != GameStage.Game)
                return;
            Player.SignUP_LMS_BR();
        }
        ...
        public void ReportCheat(C.ReportCheat p)
        {
            if (Stage == GameStage.Game)
            {
                string reason = "";
                ...
                {
                    case 1:
                        reason = "Cheat Engine running";
                        break;
                    case 2:
                        reason = "Memory Edit for Gold";
                        break;
                    case 3:
                        reason = "Memory Edit for Credit";
                        break;
                    case 4:
                        reason = "Memory Edit for Guild Gold";
                        break;
                    case 5:
                        reason = "Memory Edit for AttackTime";
                        break;
                    case 6:
                        reason = "Memory Edit for Move Time";
                        break;
                    case 7:
                        reason = "Memory Edit for Last Run Time";
                        break;
                    case 8:
                        reason = "Memory Edit for Next Run Time";
                        break;
                    case 9:
                        reason = "Memory Edit for NextMotionTime";
                        break;
                    case 10:
                        reason = "Memory Edit for NextMotionTime2";
                        break;
                    case 11:
                        reason = "Memory Edit for NextMotionTime3";
                        break;
                    case 12:
                        reason = "Memory Edit for AttackSpeed";
                        break;
                    case 13:
                        reason = "Memory Edit for Level";
                        break;
                }
                //SMain.Enqueue(string.Format("Cheater detected, Player : {0}", Player.Name));
                Player.Report.ReportCheat(reason);
            }
        }

        public void CheckClientItemInfo(C.CheckItemFileVersion p)
        {
            if (Stage != GameStage.Select)
                return;
            //  This is called
            if (p._FileTime != SMain.Envir._DatFileTime)
            {
                //  Split the data in to Chunks
                List<List<ItemInfo>> chunks = splitData(SMain.Envir._DatFileInfos.Count * 2 / 100, SMain.Envir._DatFileInfos);
                //  Check if the chunks contains only one list
                if (chunks.Count == 1)
                {
                    //  Only one chunk was required in the very unlikely case
                    Enqueue(new S.ItemInfoList { FileStart = true, fileTime = SMain.Envir._DatFileTime, ItemInfos = chunks[0], FileEnd = true });
                    return;
                }
                ...
                for (int i = 0; i < chunks.Count; i++)
                {
                    //  First List
                    if (i == 0)
                        Enqueue(new S.ItemInfoList { FileStart = true, fileTime = SMain.Envir._DatFileTime, ItemInfos = chunks[i], FileEnd = false });
                    //  Final List
        ...
        public void ChangeHeroMove(C.ChangeHeroMoveBehaviour p)
        {
            if (Stage != GameStage.Game)
                return;
            if (Player.Hero != null)
                Player.Hero.HeroBehaviour = p.heroBehaviour;
        }

        public void UseHeroSkill(C.HeroUseMagic p)
        {
            if (Stage != GameStage.Game)
                return;
            Player.UseHeroSkill(p);
        }

        public void RemoveSocket(C.RemoveRune p)
        {
            if (Stage != GameStage.Game)
                return;
            Player.RemoveSocket(MirGridType.Socketing, p.UniqueID, p.To);
        }

        public void SocketItem(C.SocketRuneStone p)
        {
            if (Stage != GameStage.Game)
                return;
            Player.SocketItem(p.Item, p.slot, p.UsingItem);
        }

        public void GetSocketInfo(C.RequestSocketInfo p)
        {
            if (Stage != GameStage.Game)
                return;
            Player.GetSocketInfo(p.item);
        }

        public void GetHeroRanks(C.GetHeroRank p)
        {
            if (Stage != GameStage.Game)
                return;
            Player.GetHeroRank(p.RType);
        }
        public void CreateHero(C.HeroCreation p)
        {
            //return;//   disabled
            if (Stage != GameStage.Game)
                return;
        ...
        public void PurchaseRecipe(C.BuyRecipe p)
        {
            if (Stage != GameStage.Game)
                return;
            Player.PurchaseRecipe(p);
        }
        public void RecipieShop(C.GetRecipeShop p)
        {
            if (Stage != GameStage.Game)
                return;

            Player.RequestRecipieShop(p);
        }

        public void CollectCrafting(C.CollectCraft p)
        {
            if (Stage != GameStage.Game)
                return;
            Player.CheckCraftProgress(p.Rcipe);
        }
        ...
        private void StartGame(C.StartGame p)
        {
            ...
            {
                Enqueue(new S.StartGame { Result = 0 });//返回策略不允许
                return;
            }

            if (Account == null)
            {
                Enqueue(new S.StartGame { Result = 1 });//返回账号不存在
                return;
            }
            ...
            if (info.Banned)
            {
                ...
                {
                    Enqueue(new S.StartGameBanned { Reason = info.BanReason, ExpiryDate = info.ExpiryDate });//返回被封号
                    return;
                }
                info.Banned = false;
            }
        ...
        public void StashHero(C.StashHero p)
        {
            if (Stage != GameStage.Game) return;

            Player.StashHero(p.Name);
        }
        ...
        private void InventoryToHeroInventory(C.InventoryToHeroInventory p)
        {
            if (Stage != GameStage.Game) return;
            if (Player.GainedHero && Player.Hero != null)
            Player.InventoryToHeroInventory(p.From, p.To);
        }
        ...
        private void HeroInventoryToInventory(C.HeroInventoryToInventory p)
        {
            if (Stage != GameStage.Game) return;

            Player.HeroInventoryToInventory(p.From, p.To);
        }
        ...
        private void HeroEquipItem(C.HeroEquipItem p)
        {
            if (Stage != GameStage.Game) return;

            Player.HeroEquipItem(p.Grid, p.UniqueID, p.To);
        }
        ...
        private void HeroRemoveItem(C.HeroRemoveItem p)
        {
            if (Stage != GameStage.Game) return;

            Player.HeroRemoveItem(p.Grid, p.UniqueID, p.To);
        }
        ...
        private void HeroUseItem(C.HeroUseItem p)
        {
            if (Stage != GameStage.Game) return;

            if (Player.GainedHero && Player.Hero != null)
                Player.Hero.UseItem(p.UniqueID);
        }
        ...
        private void Inspect(C.Inspect p)
        {
            ...
            else if (p.Ranking && p.Hero)
                Player.Inspect((int)p.ObjectID, p.HeroName);
            else
                Player.Inspect(p.ObjectID);
        }


        private void HeroRunTo(C.HeroRunTo p)
        {
            if (Stage != GameStage.Game) return;

            Player.HeroRunTo(p.GoTo);
        ...
        private void ChangeHMode(C.ChangeHMode p)
        {
            if (Stage != GameStage.Game) return;

            Player.HMode = p.Mode;

            if (Player.Hero != null && Player.GainedHero && Player.Hero.isSpawn)
            {
                if (Player.HMode == HeroMode.Guard)
                {
                    var guardPos = new GuardSpot()
                    {
                        GuardMap = Player.CurrentMap,
                        GuardLocation = Player.CurrentLocation,
                    };

                    Player.Hero.guardSpot = guardPos;
                }
                else
                    Player.Hero.guardSpot = null;

            }

            Enqueue(new S.ChangeHMode { Mode = Player.HMode });
        }
        ...
        private void AutoPotUpdate(C.AutoPotUpdate p)
        {
            if (Stage != GameStage.Game) return;

            if (!Player.GainedHero || Player.Hero == null) return;

            foreach(var pot in Player.Hero.autoPotSystem.AutoPotList)
            {
                switch (pot.AutoPotType)
                {
                    case 1:
                        pot.ItemIndex = p.HpInfo;
                        pot.PercentTrigger = p.HpRate;
                        break;

                    case 2:
                        pot.ItemIndex = p.MpInfo;
                        pot.PercentTrigger = p.MpRate;
                        break;
                }
            ...
            private void HeroMagicKey(C.HeroMagicKey p)
        {
            if (Stage != GameStage.Game) return;

            if (!Player.GainedHero || Player.Hero == null) return;

            if (Player.Hero.Magics.Count(x => x.Key > 1) >= 7)
                return;

            for (int i = 0; i < Player.Hero.Magics.Count; i++)
            {
                UserMagic magic = Player.Hero.Magics[i];
                if (magic.Spell != p.Spell)
                    continue;
        ...
        private void ConsignItem(C.ConsignItem p)
        {
            ...
            Player.ConsignItem(p.UniqueID, p.Price , p.GGPrice);
        }
        private void MarketSearch(C.MarketSearch p)
        {
            ...
            Player.MarketSearch(p.Match , p.Type, p.MinShape, p.MaxShape, p.Class);
        }
        private void MarketRefresh(C.MarketRefresh p)
        {
            if (Stage != GameStage.Game) return;

            Player.MarketRefresh(p.Type);
        }
        ...
        private void MarketBuy(C.MarketBuy p)
        {
            ...
            Player.MarketBuy(p.AuctionID,p.MailItems,p.ggPay);
        }
        ...
        private void GetCrafts(C.GetRecipes p)
        {
            Player.GetRecipes(p.IType, p.MinShape, p.MaxShape);
        }

        private void DoCraft(C.RequestCraft p)
        {
            Player.Crafting(p.Recipe);
        }
        ...
        private void ComboSpell(C.ComboSpell p)
        {
            if (Stage != GameStage.Game) return;
            Player.DoComboSpell(p.spell);
        }
