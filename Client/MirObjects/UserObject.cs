...
{
    /// <summary>
    /// 地图对象的行为来自于用户输入或者 AI 行为逻辑，在这里我们需要对用户控制的角色和其他对象做一个区分，
    /// 其他角色的行为序列是通过服务端同步到客户端的，而用户输入是由输入设备直接产生的，需要加以限制，
    /// 因此在 UserObject 上有一个 QueuedAction 属性用于记录下一个待执行的行为，
    /// 同时这也限制了游戏中的角色无法一边移动一边施法
    /// </summary>
    public class UserObject : PlayerObject
    {
        ...
        {
            //代码可见 UserObject 并不直接处理 Action，
            //而是只处理从用户输入产生 QueuedAction 到 QueuedAction 加入到行为队列这一过程，
        ...
        {
            //每一个地图对象 MapObject 在帧开始时都会被调用到 Process 方法处理游戏逻辑，
            //游戏角色 UserObject 继承了 PlayerObject，
            //它重写了 PlayerObject 的 ProcessFrames 方法来处理用户输入产生的 QueuedAction
            bool clear = CMain.Time >= NextMotion;

            //这里的 base.ProcessFrames 用于处理动画帧的更新，我们先重点来看 SetAction 的执行逻辑，
            //当目前的行为是 Idle 类状态（站立和野蛮冲撞失败等），
            //且存在 QueuedAction 或 ActionFeed 不为空时则执行 SetAction 执行角色的下一个行为
            base.ProcessFrames();
