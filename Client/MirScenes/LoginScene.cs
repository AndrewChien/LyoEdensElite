...
namespace Client.MirScenes
{
    public sealed class LoginScene : MirScene
    {
        ...
        public LoginScene()
        {
            ...
            int w = 0;
            int h = 0;
            SoundManager.PlaySound(SoundList.IntroMusic, true);     //注意此处（SoundManager初始化）首次加载SoundList声音库
            ...
            _backgroundFire = new MirAnimatedControl
            {
                ...
                Library = Libraries.CustomLoginScene,   //注意此处首次加载Libraries画面库
                Loop = true,
                Parent = this,
                Visible = true
            };
        ...
        public override void ProcessPacket(Packet p)
        {
            switch (p.Index)
            {
                case (short)ServerPacketIds.Connected://首次连接包
                    Network.Connected = true;
                    SendVersion();//与服务端连接后，发送数据完整性检查请求（这里会对 Executable 进行 hash）
                    break;
                case (short)ServerPacketIds.ClientVersion://服务端校验客户端版本结果包
                    ClientVersion((S.ClientVersion) p);
                    break;
                case (short)ServerPacketIds.NewAccount://新建账号包
                    NewAccount((S.NewAccount) p);
                    break;
                case (short)ServerPacketIds.ChangePassword://修改密码包
                    ChangePassword((S.ChangePassword) p);
                    break;
                case (short)ServerPacketIds.ChangePasswordBanned://修改密码账号被封包
                    ChangePassword((S.ChangePasswordBanned) p);
                    break;
                case (short)ServerPacketIds.Login://登录失败包
                    Login((S.Login) p);
                    break;
                case (short)ServerPacketIds.LoginBanned://登录账号被封包
                    Login((S.LoginBanned) p);
                    break;
                case (short)ServerPacketIds.LoginSuccess://登陆成功包
                    Login((S.LoginSuccess) p);
                    break;
                default:
                    base.ProcessPacket(p);//其他包
                    break;
            }
        }
        ...
        private  void SendVersion()
        {
            ...
            try
            {
                //此处对Executable进行hash，校验客户端是否遭到篡改
                byte[] sum;
        ...
        private void Login(S.LoginSuccess p)
        {
            ...
            _backgroundDoor.AfterAnimation += (o, e) =>
            {
                ...
                //注意：登陆成功后，此处将ActiveScene更改：LoginScene-->SelectScene ！！！登陆成功直接返回角色列表
        ...
        public sealed class LoginDialog : MirImageControl
        {
            ...
            private void Login()
            {
                ...
                Network.Enqueue(new C.Login {AccountID = AccountIDTextBox.Text, Password = PasswordTextBox.Text});//此处明文传输用户名密码
            }
