...
namespace Client.MirScenes
{
    ...
    public sealed class GameScene : MirScene
    {
        ...
        {
            //传奇的渲染管线设计的还是比较简单的，从大流程上而言主要分为两步渲染：
            // 1、Draw Game Scene 分层渲染游戏场景
            if (MapControl != null && !MapControl.IsDisposed)
                MapControl.DrawControl();

            // 2、Draw GUI  渲染游戏的 GUI
            base.DrawControl();
        ...
        {
            //对于移动逻辑本身而言，Walking 只是沿着瓦片地图的 X 或者 Y 方向行走步长 Step 个 Cell，
            //但对于客户端渲染而言，行走是一个帧动画，我们需要将行走过程连贯的渲染出来，否则角色的移动就像是在地图上进行瞬移。
            //本地处理 Walking Action 的过程中，我们已经完成了 CurrentAction 和 Frame 的设置，
            //接下来的逻辑位于 PlayerObject 的 Process 中，在这里我们会根据步长计算出位移，再根据 Frame 将位移分解到每一帧来产生与行走动画同步的位移
        ...
        public override void ProcessPacket(Packet p)
        {
            switch (p.Index)
            {
                ...
                case (short)ServerPacketIds.MapInformation: //MapInfo
                    ...
                    break;
                case (short)ServerPacketIds.UserInformation://刚进入游戏时，服务端发送该包
        ...
        {
            //当服务端接收到 C.Walk 并完成相应处理后，会发送 S.UserLocation 到客户端，
            //客户端在收到该数据包后会将 MapControl.NextAction 置为 0 使得客户端可以立即执行下一个行为
...
if (realItem.Type == ItemType.Potion && realItem.Shape == 0)
{
    ...
    public sealed class MapControl : MirControl
    {
        ...
        //传奇采用了 Tilemap 地图，按照从左到右、从上到下的顺序进行绘制，每个 Tile 的固定大小为 48 x 32 px
        ...
        public MapControl()
        {
            ...
            //首先根据分辨率计算 x 和 y 轴方向所需的 Tile 数量，这里的 OffsetX 和 OffsetY 分别是半屏下的 Tile 数量
            OffSetX = Settings.ScreenWidth / 2 / CellWidth;
            OffSetY = Settings.ScreenHeight / 2 / CellHeight - 1;

            //为了避免出现黑边加上 4 作为 x, y 方向的 half extent
        ...
        /// <summary>
        /// 【核心】游戏场景画面渲染过程
        /// （通过 CreateTexture 离屏渲染到 ControlTexture）
        /// </summary>
        ...
        protected internal override void DrawControl()
        {
            //传奇设计了帧缓存 ControlTexture，在不需要播放动画和移动物体的情况下可以复用之前的渲染结果
            ...
            //这里的 SetOpacity 实际上是调整 BlendMode，
            //由于 MapControl 在初始化时采用的 Opacity 默认值为 1.0，
            //因此会将 BlendMode 设置为 SourceAlpha InverseSourceAlpha，
            //即通过 dst.rgb = src.rgb * src.a + dst.rgb + (1 - src.a) 进行颜色混合
        ...
        {
            //在传统的 Tilemap 渲染中，地图资源会以地图元数据 + 瓦片贴图的形式存在，
            //传奇作为一款网络游戏，地图元数据存储在服务端，瓦片贴图存储在客户端。

            //Direct 3D9 的 Sprite 的 UV 坐标系原点为左上角，向右为 x 轴，向下为 y 轴，
            //传奇采用了 Tilemap 地图，
            //按照从左到右、从上到下的顺序进行绘制，每个 Tile 的固定大小为 48 x 32 px，
            //首先根据分辨率计算 x 和 y 轴方向所需的 Tile 数量，
            //这里的 OffsetX 和 OffsetY 分别是半屏下的 Tile 数量，
            //为了避免出现黑边加上 4 作为 x, y 方向的 half extent
            ...
            //子图层1，以该Back层为例，说明地板的贴图绘制原理：
            //Tilemap 的绘制采用了双层循环，以用户当前坐标为中心，
            //向左减去 ViewRangeY 得到 minY，向右加上 ViewRangeY 得到 maxY，
            //在内层循环中则是从 minX 迭代到 maxX，
            //因此贴 Tile 的顺序为自上而下、从左到右

            //需要注意的是：
            //循环中的 x, y 实际上是 Tile 坐标，在绘制时我们需要将其换算为屏幕坐标，
            //换算的过程很简单，只需要将 Tile 坐标乘以 CellWH 即可。
            //通过 Tile 坐标可以在 Library 中查询 Cell 信息，获取到图片的 Index 并完成绘制，
            //这里有一个细节是实际的 Tile 分辨率为 96 x 64，是 CellWH 48 x 32 的 2 倍，
            //因此在贴 Tile 时在 x, y 方向都会产生 50% 的覆盖，这应该是为了避免出现裂缝：
            //当用户移动时，地图会沿着相反的方向移动从而产生用户在地图上移动的视觉效果，
            //当然我们也可以理解为游戏的 Main Camera 与用户绑定，地图在不断地被变换到观察空间。
        ...
        {
            //遮挡关系处理:
            //逻辑顺序上是先绘制的地图元素，后绘制的人物，但人物却可以被地图的元素挡住
            //按照先绘制 Tile，再绘制 Objects 的顺序，理论上 Object 会挡住 Tile 从而产生错误的视觉效果，
            //但实际上 Player 可以正确被景物遮挡

            //默认配置下，同样的 z-order 后绘制的 Sprite 会遮挡前面绘制的 Sprite，
            //里看起来非常反直觉的现象实际上是美术资产上做的 trick，在角色和景物的时候有一个 Y 方向向上的偏移，
            //而景物绘制的时候这个向上偏移的 Y 更大，使得角色实际上被绘制的比同一行的 Tile 要更早

            //但这还不是全部仔细观察传奇的地图可以发现，当角色完全被景物挡住时，
            //会有一个半透明的身影被渲染出来以避免玩家看不到自己的角色（仔细看树后的角色）

            //这是怎么实现的呢？实际上在 DrawObjects 对 Tile 和 Objects 的绘制结束后，
            //会开启 AlphaBlend，将 SourceFactor 指定为 0.4 对玩家的角色进行一次透明度混合

            //技能与特效的透明度混合（unity）：
            //在上面的渲染中，地图和角色的透明度混合大都采用了 SourceAlpha InverseSourceAlpha 的常规混合方式，
            //而对于场景中的技能和特效，为了让他们变得更亮，需要采用 Additive 的混合方式，即 SourceAlpha One:
            //Device.SetRenderState(RenderState.AlphaBlendEnable, true);
            //Device.SetRenderState(RenderState.SourceBlend, Blend.SourceAlpha);
            //Device.SetRenderState(RenderState.DestinationBlend, Blend.One);
            //注意这里的 DestinationBlend 一定要设置为 One，不可以为 InverseSourceAlpha，
            //这是因为资产中的特效的 Alpha 通道在很大的区以内都接近 1，如果采用 InverseSourceAlpha 会在地图上产生黑色背景
            ...
            for (int y = User.Movement.Y - ViewRangeY; y <= User.Movement.Y + ViewRangeY + 25; y++)
            {
                ...
                for (int x = User.Movement.X - ViewRangeX; x <= User.Movement.X + ViewRangeX; x++)
                {
                    ...
                    bool blend;
                    Size s;
                    #region Draw shanda's tile animation layer 绘制头衔
                ...
                (MapObject.User.Direction == MirDirection.Left))
            {
                ...
                if (MapObject.User.Helmet > 0)
                    MapObject.User.DrawHat();//绘制帽子wrong order :P
                //绘制翅膀
        ...
        private void CheckInput()
        {
            ...
            if (MouseControl == this || MouseControl == GameScene.Scene.MainDialog)
            {
                ...
                if (AutoRun)
                {
                    ...
                    //当用户通过鼠标右键进行移动时，将会产生一个 Walking Action 设置到 QueuedAction
