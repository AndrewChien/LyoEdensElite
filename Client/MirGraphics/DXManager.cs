...
namespace Client.MirGraphics
{
    class DXManager
    {
        ...
        public static Texture RadarTexture, PlayerRadarTexture;
        public static List<Texture> Lights = new List<Texture>();
        public static List<Texture> RectangleLights = new List<Texture>();
        ...
        public static Point[] LightSizeRectangle =
{
            new Point(125,125),
            new Point(205,205),
            new Point(285,285),
        ...
        private static unsafe void LoadTextures()
        {
            ...
            CurrentSurface = MainSurface;//注意，此处将CurrentSurface设置为MainSurface，即BackBuffer
            Device.SetRenderTarget(0, MainSurface);

            if (PlayerRadarTexture == null || PlayerRadarTexture.Disposed)
            {
                //  This draws it as a rectangle, 4 wide 4 high
                PlayerRadarTexture = new Texture(Device, 4, 4, 1, Usage.None, Format.A8R8G8B8, Pool.Managed);

                using (GraphicsStream stream = PlayerRadarTexture.LockRectangle(0, LockFlags.Discard))
                using (Bitmap image = new Bitmap(4, 4, 16, PixelFormat.Format32bppArgb, (IntPtr)stream.InternalDataPointer))
            ...
            CreateLights();
            CreateRectangleLights();
        }

        /// <summary>
        /// 创建光线区域
        /// </summary>
        private unsafe static void CreateRectangleLights()
        {

            for (int i = RectangleLights.Count - 1; i >= 0; i--)
                RectangleLights[i].Dispose();

            RectangleLights.Clear();

            for (int i = 1; i < LightSizeRectangle.Length; i++)
            {
                // int width = 125 + (57 *i);
                //int height = 110 + (57 * i);
                int width = LightSizeRectangle[i].X;
                int height = LightSizeRectangle[i].Y;
        ...
        {
            /*
                这里实际上就是将 RT0 绑定到目标纹理，传奇没有采用 multi target 模式，
            因此默认情况下片段着色器的输出将直接通过 RT0 绑定的 Attachment 进行输出。
            再回去看一眼渲染循环中的代码(DXManager.cs-LoadTextures())会发现 RT0 会被绑定
            到 DXManager.MainSurface，而 MainSurface 实际上就是 BackBuffer，
            因此渲染内容会在 swap 后直接上屏
             */
        ...
        public static void AttemptReset()
        {
            try
            {
                Device.CheckCooperativeLevel(out int result);
